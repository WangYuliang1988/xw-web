// 前端工程 xw-client 自动化回滚脚本

const { question } = require('./utils/question');
const { exec, login } = require('./utils/connect');
const { REMOTE_BASE_DIR, REMOTE_SRC_DIR } = require('./utils/constants').client;

login(async () => {
  // 获取可用工程列表，列表按工程名称正排序
  let rawPros = await exec(`ls ${REMOTE_BASE_DIR} -p -1`);
  let proArr = rawPros.split('\n').filter(item => {
    return item.startsWith('xw-client-') && item.endsWith('/');
  }).map(item => {
    return item.substring(0, item.length - 1);
  });

  // 获取当前xw-client软连接指向的目录
