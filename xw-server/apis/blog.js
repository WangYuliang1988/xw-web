...
const blogApis = [
  ...
  },
  {
    ...
    fn: async (ctx) => {
      ...
      let { name, author, dynasty, summary, content } = ctx.request.body;

      if (!name || !name.trim()) {
        throw new APIError("param:invalid_value", "invalid name");
      }
      if (!author || !author.trim()) {
        throw new APIError("param:invalid_value", "invalid author");
      }
      if (!dynasty || !dynasty.trim()) {
        throw new APIError("param:invalid_value", "invalid dynasty");
      }
    ...
    fn: async (ctx) => {
      ...
      if (!blog) {
        throw new APIError(
          ...
          `fail to update the blog with id '${id}', maybe the id is not correct`
        );
      }
      ctx.body = blog;
    },
    ...
    fn: async (ctx) => {
      ...
      if (rs === 0) {
        throw new APIError(
          ...
          `fail to delete the blog with id '${id}', maybe the id is not correct`
        );
      }
